...
export const EditarCliente = () => {
	...
	const onSubmit = handleSubmit(
		async ({ nombre,apellido,total_pagar,barras, localidad,kilos }) => {
			axios.put(`${import.meta.env.VITE_API_URL}/clientes/${params.id}`, {
				data: {
                    nombre:nombre,apellido:apellido,total_pagar:total_pagar,barras:barras, localidad:localidad,kilos:kilos
				},
			});
	...
	useEffect(() => {
		async function data() {
			...
			setValue('apellido', res.data.data[0].attributes.apellido);
			setValue('total_pagar', res.data.data[0].attributes.total_pagar);
			setValue('total_barras', res.data.data[0].attributes.barras);
			setValue('localidad', res.data.data[0].attributes.localidad);
			setValue('kilos', res.data.data[0].attributes.kilos);
