...
export const NuevoClienteAccesorio = () => {
  const [nombre, setNombre] = useState("");
  const [apellido, setApellido] = useState("");
  const [localidad, setLocalidad] = useState("");
  const [total_pagar, setTotalPagar] = useState(0);
  const [total_accesorios, SetTotalAccesorios] = useState(0);
  ...
  const handleSubmit = (e) => {
    ...
    if ([nombre, apellido, localidad].includes("")) {
      ...
      axios.post(`${import.meta.env.VITE_API_URL}/clientes-accesorios`, {
        data: {
          nombre: nombre,
          apellido: apellido,
          localidad: localidad,
      ...
      >
        ...
        <div className="flex gap-2 ">
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            <input
              type="number"
              value={total_accesorios}
              onChange={(e) => SetTotalAccesorios(e.target.value)}
              className="px-6 py-3 rounded-lg bg-white outline-none placeholder:text-black/60 w-auto"
        ...
        <input
          ...
          value={"CARGAR CLIENTE NUEVO - ACCESORIO"}
        />
      </form>
    </div>
  );
