...
export const Facturar = () => {
	...
	// console.log(perfilId.map(categoria => categoria.attributes.categoria));
	...
	// 	const columns = ['NUMERO', 'CLIENTE', 'LOCALIDAD'];
	// 	const columnsTwo = ['CANTIDAD BARRAS', 'CODIGO', 'KILOS', 'CATEGORIA'];
	...
	// 		[
	// 			`${clienteId[0]?.id}`,
	// 			`${clienteId[0]?.attributes.nombre} ${clienteId[0]?.attributes.apellido}`,
	// 			`${clienteId[0]?.attributes.localidad}`,
	// 		],
	// 	];
	// 	const dataTwo = perfilId.map(p => [
	// 		`${p.attributes.barras}`,
	// 		`${p.attributes.codigo}`,
	// 		`${p.attributes.kg}`,
	// 		`${p.attributes.categoria.toUpperCase()}`,
	// 	]);
	...
	// 	//guardar el pdf nombre especifico
	// 	doc.save(`factura_${clienteId[0]?.id}.pdf`);
	// };
	...
	// console.log(perfilId.map(perfil => `${perfil.attributes.barras}`));

	// console.log(`${perfilId.map(c => c.attributes.cantidad)}`);
	...
	return (
		<div className="py-[150px] px-2 max-md:py-[60px]">
			...
			<div className="bg-primary p-10 rounded-lg w-1/2 max-md:w-full mx-auto shadow-2xl shadow-black/30 space-y-6 relative">
				...
				>
					...
					<div className="flex gap-3 items-center">
						...
						{/* <div>{totalKgHerrero() + totalKgModena()}</div> */}
					</div>
